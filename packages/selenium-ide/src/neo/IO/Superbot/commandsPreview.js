const removeModal = () => {
  const modal = document.getElementById('command-preview');
  if(modal !== null){
    modal.remove();
  }
}

const commandPreview = command => new Promise(async () => {
  const elem = document.getElementById('command-preview');
  if(elem !== null){
    elem.remove();
  }
  ...
  const domIframe = document.getElementById('command-preview');
